...
**Failure-mode coverage for gating workflows (the convention, [#313](https://github.com/devantler-tech/reusable-workflows/issues/313)):** every **gating** reusable workflow — one whose job is to *fail a PR on bad input* — carries **both** a *passes-on-good-input* and a *blocks-on-bad-input* self-test, because a happy-path test alone cannot catch a gate that silently stopped biting. The pattern (per `test-govulncheck-strict-blocks` and `test-zizmor-blocks`): point the gate's **own** action — pinned to the **same SHA**, guarded by a `*-action-lockstep` check — at a deliberately-bad fixture under `tests/`, `continue-on-error`, then assert the run *failed* **and** reported the expected finding (so an operational error can't false-pass). The fixture lives **outside** the gate's own scan scope (e.g. `tests/zizmor-fixture/` is outside `.github/workflows/`, so zizmor's canonical discovery ignores it) so it never trips the real gate. **Non-gating** workflows (release/publish/deploy dry-runs, `delete-workflow-runs`, `enable-auto-merge`, `template-sync`, `sync-cluster-policies`, `update-agent-skills`, `scan-for-todo-comments`) have no "bad input" to reject, so a happy-path `[Test]` job is complete coverage for them. Where a clean failure-mode fixture is genuinely impractical (e.g. `dependency-review` needs a PR diff introducing a bad dependency), record the reasoned gap rather than forcing a fragile test.
